...
/**
 * Modified Gaussian Elimination Class.
...
public class ComputeGaussian {
    ...
    public int[] solveEquation(int[][] B, int[] b) {
        ...
        try {
            ...
            for (int i = 0; i < b.length; i++) {
                A[i][b.length] = gf.add(b[i], A[i][b.length]);
            }
    ...
    protected void substitute() throws IllegalStateException {
        ...
        temp = gf.inverse(A[A.length - 1][A.length - 1]);
        if (temp == 0) {
            throw new IllegalStateException("The equation system is not solvable");
        }

        /** backward substitution **/
        x[A.length - 1] = gf.mult(A[A.length - 1][A.length], temp);
        for (int i = A.length - 2; i >= 0; i--) {
            tmp = A[i][A.length];
            for (int j = A.length - 1; j > i; j--) {
                temp = gf.mult(A[i][j], x[j]);
                tmp = gf.add(tmp, temp);
            }

            temp = gf.inverse(A[i][i]);
            if (temp == 0) {
